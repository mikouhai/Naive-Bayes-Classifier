...
# Student Name: Raymond Huang
# UW Email    : rhuang43@ucsc.edu
...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        """
        # print(self.word_set(train_spams[0]))
        self.num_train_spams = len(train_spams); self.num_train_hams = len(train_hams); checker = []
        ...
        def get_counts(filenames:list):
            ...
            for i in filenames:
                i = i.lower()
                if i not in checker: checker.append(i)
                for count in range(0, len(checker)):
                    if turn == 0:
                        if checker[count] in self.word_counts_spam: self.word_counts_spam[checker[count]] += 1
                        else: self.word_counts_spam[checker[count]] = 1
                    else:
                        if checker[count] in self.word_counts_ham: self.word_counts_ham[checker[count]] += 1
                        else: self.word_counts_ham[checker[count]] = 1
        for num in range(0, len(train_spams)):
            turn = 0
            get_counts(self.word_set(train_spams[num]))
            checker = []
        for num in range(0, len(train_hams)):
            turn = 1
            get_counts(self.word_set(train_hams[num]))
            checker = []
    ...
    def predict(self, filename:str):
        ...
        """
        pass # TODO: Your code here (10-20 lines)
        p_hams = (self.num_train_hams) / (self.num_train_hams + self.num_train_spams); toham = 0
        p_spams = (self.num_train_spams) / (self.num_train_spams + self.num_train_hams); tospam = 0;
        probS = 0; probH = 0; checker = []; x = self.word_set(filename)
        for i in x:
            i = i.lower()
            if i not in checker:
                checker.append(i)
        for i in checker:
            if i in self.word_counts_spam:
                probS += np.log(self.word_counts_spam[i] + 1 / self.num_train_spams + 2)
            else:
                probS += 1 / 2
        for i in checker:
            if i in self.word_counts_ham:
                probH += np.log(self.word_counts_ham[i] + 1 / self.num_train_hams + 2)
            else:
                probH += 1 / 2
        tospam = np.log(p_spams) + probS; toham = np.log(p_hams) + probH
        return self.SPAM_LABEL if tospam > toham else self.HAM_LABEL
